...
function [FvsZp, coatPullSol, compZpRng] = loopCoatPullRing(alpha, mesh, lambda, acoat, rF, rIn, rOut, zpRng, f0, k0, dk, P, gamma, C0, R0, initSol)
...
for ii = 1:length(zpRng)
    ...
    [~,Sol,f] = coatPullRing(alpha, mesh, lambda, acoat, rF, rIn, rOut, zpRng(ii), f0, k0, dk, P, gamma, C0, R0, initSol);
...
aF = ((rF/R0)^2)/2;
aIn = t(find(Sol(1,:)*R0>=rIn,1));
aOut = t(find(Sol(1,:)*R0<=rOut,1,'last'));
coatArea = [0 acoat];
